...
| 3 — Real-time ingestion | Kafka/syslog sources feeding the window builder | 🟡 `SyslogTailSource` tails syslog-format files into the live engine; `POST /v1/events` pushes events through the push engine over REST; offline threat-intel feed loading from a local file (`SENTINEL_THREAT_FEED_FILE`, refreshed out-of-band) with graceful degradation; Kafka transport is future work |
| 4 — Enhanced detection | Scan classification, C2 from DNS/TLS, phishing, insider | 🟡 Nine detectors plus threat-intel enrichment: C2/exfil scores rise when a destination matches a loaded feed (URLhaus-format, TTL-bound, list evidence — not verdicts); validated quiet-on-benign, not on real attack data |
